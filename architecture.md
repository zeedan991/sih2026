...
- The separate September 3 five-fold study refits every preprocessing component inside each fold and uses 100 epochs for all VQCs. With the live-size 20-row quantum training constraint, the six-model ensemble measured **91.57% mean accuracy, 83.50% malignant sensitivity, 96.36% specificity, and 0.981 mean ROC-AUC**. Full-feature Logistic Regression measured 97.37%, 94.36%, 99.16%, and 0.995; same-four Logistic Regression measured 94.03%, 90.59%, 96.08%, and 0.989. The amplitude/2-qubit QSVM was unstable (54.61% mean accuracy, 29.20–62.28% range). This is retained as stress-test/generalization evidence and a limitation, not mixed with the 200-row Phase 2 headline.
...
**Generalization actually measured:** independent stratified holdouts across seeds 42, 123, and 2026, plus a five-fold stratified study. Each fold rebuilds imputation, standard scaling, ANOVA feature selection, and quantum-range scaling using only that fold's training rows. The six-member quantum ensemble is then trained with 100 epochs per VQC. The same-feature comparison controls feature count, not every experimental factor: the live and fold studies use a 20-row quantum pool, the older saved Phase 2 study uses 200, and classical fold models use all available fold-training rows. OOB remains model-level inside each fold: preprocessing is fold-isolated but shared across a fold's bootstraps. One benchmark dataset still does not establish clinical or external-cohort generalization.
