...
### D-28. Multi-disease means independently trained modules, not one universal diagnosis model

**Context:** SIH26139 asks for a platform applied to biomedical datasets and gives cancer, cardiovascular disorders, and neurological conditions as examples. It does not require, and the available evidence cannot support, a single model that detects every disease. The first release demonstrated only WBCD breast-mass classification, which met the minimum but left platform scalability mostly architectural.

**Decision:** Add UCI Early Stage Diabetes Risk Prediction as a second, separately trained module under CC BY 4.0. Both modules use the same complete workflow: train-only preprocessing, `SelectKBest(f_classif, k=4)`, inward quantum scaling, four VQCs, angle/4-qubit and amplitude/2-qubit QSVMs, OOB weighted voting, all four classical baselines in full/same-four configurations, and SHAP/LIME with real feature names. Disease labels, schemas, range checks, evidence, and reports remain module-specific. Never pitch this as “detects any disease”; pitch it as a reusable platform demonstrated on two different biomedical modalities (numeric measurements and a signs/symptoms questionnaire). A third module still requires its own dataset review, training, validation, and clinical framing.

**Limitations:** The diabetes source has 520 records from a single hospital questionnaire study in Bangladesh. Strong symptom variables can produce unusually high apparent accuracy, the source's binary sex/gender field is limited, and neither external Indian-cohort nor prospective validation exists. The generated report therefore includes a small held-out subgroup audit and these limitations; it does not convert model confidence into clinical risk.

**Measured outcome:** Under the deliberately small live-size 20-row quantum training pool, three independent 100-epoch seeds produced 83.01% mean ensemble accuracy (76.92–92.31%), 81.25% condition sensitivity, and 85.83% specificity. Full-feature classical models ranged from 90.06% mean (LogReg) to 95.51% (SVM); same-four classical models were 85.58–86.54% mean. One VQC was unstable at 58.01% mean (39.42–69.23%) and was nearly zero-weighted on its worst OOB run. This second module demonstrates reuse, not quantum superiority; exact runs and weights are retained in `artifacts/evaluation/early_diabetes_three_seed.json`.

---

### D-29. “AI report” is local evidence synthesis, not an unconstrained medical chatbot

**Context:** A report is more useful in a judge demo than a bare class label, but adding a hosted LLM would introduce cost, network/API-key dependence, patient-data transfer, non-determinism, and hallucination risk. It would not make the underlying prediction more valid. The project already contains AI/ML in its VQCs, quantum kernels, classical baselines, SHAP, and LIME.

**Decision:** `/report` recomputes the selected module's real prediction and produces a structured, deterministic evidence summary: both paradigms, both classical feature views, agreement/disagreement, selected raw features, all six quantum member outputs and OOB weights, held-out sensitivity/specificity, runtime configuration, dataset citation/license, subgroup audit where available, and limitations. The judge UI downloads a self-contained HTML file or opens the browser's Print/Save-PDF flow. Everything runs locally with the pinned free stack; no external AI API receives patient data. Copy must say “screening signal” and “model confidence,” never diagnose, prescribe, or call confidence a personal disease probability.
