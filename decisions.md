...
### D-23. LIME direction is an observed-patient contribution, not a raw local slope

**Context:** LIME's continuous tabular explanation returns coefficients from a local linear surrogate fitted in standardized feature space. A coefficient's sign answers "what happens as this feature increases locally." SHAP's sign instead answers "how did this patient's observed value move the prediction relative to the background." Comparing those raw signs is invalid: on a benign patient, a low tumour-size value can contribute positively toward benign in SHAP even though LIME correctly gives tumour size a negative local slope.

**Verification:** The first real-model Phase 3 run exposed exactly that apparent all-feature sign disagreement while retaining complete top-feature overlap. Reconstructing the LIME patient contribution as `local_coefficient * standardized_patient_displacement` made the quantities comparable. On the repeated real VQC-heavy explanation, all four selected clinical features overlapped and all four directions agreed; SHAP and the reconstructed LIME surrogate each had zero additivity residual.

**Decision:** Store and compare LIME's per-patient contributions, not bare local slopes. Keep the surrogate intercept, local prediction, reconstructed prediction, and residual in the structured result so this conversion remains auditable. LIME disagreement is still reportable when it occurs; the implementation must never force agreement.
