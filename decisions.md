...
**Context:** The quantum-range `MinMaxScaler` correctly maps the training-row minima to zero. On each Phase 2 benchmark split, one real row is the minimum of all four selected features and therefore becomes the exact vector `[0, 0, 0, 0]`. A zero vector has no direction and cannot be normalized into a quantum amplitude state; passing it directly to `AmplitudeEmbedding(normalize=True)` produces invalid values. The earlier architecture note that `normalize=True` alone handled every raw feature vector missed this boundary case.

**Decision:** Only for the 2-qubit amplitude kernel, map an exact all-zero row to the canonical computational basis vector `[1, 0, 0, 0]` (the `|00>` state) before `AmplitudeEmbedding`. Nonzero rows are unchanged and continue to use `normalize=True, pad_with=0.0`. `tests/test_qsvm.py` asserts that a kernel matrix containing the real pipeline edge shape stays finite with a unit diagonal. This is a local representation safeguard, not a change to feature selection, scaling, labels, or qubit count.
...
**Decision:** Keep the honest OOB scheme and all six required models; do not tune weights against the test set to force the roadmap target. The Phase 2 exit target (ensemble accuracy at least the best single model) was not met on the across-seed mean by 0.29 percentage points. This is not treated as a broken implementation because every family reproduced its expected performance band, training costs decreased, the weak amplitude model was correctly down-weighted, and the difference is one held-out patient on each non-winning seed. Any later weighting change must be selected on training/OOB data and re-evaluated on untouched test data.
